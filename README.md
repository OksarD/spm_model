...
The device uses a belt-driven concentric pulley stack to achieve coaxial motion, and features press-fir ball bearings to ensure rigid kinematic motion with no backlash.
...
The device is sent a reference position and velocity (yaw-pitch-roll) in real-time over USB/UART, and controls with a velocity feedforward controller, with 3 PID loops for compensation on each axis.
It also has position control for homing and initialisation.
